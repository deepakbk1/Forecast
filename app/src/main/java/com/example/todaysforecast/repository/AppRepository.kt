...
class AppRepository @Inject constructor(private val apiService: ApiService) :
    ...
    suspend fun getWeather(lat: Double, lon: Double, unit: String) =
        getResult { apiService.getWeather(lat, lon, unit) }

    suspend fun getFiveDayForecast(lat: Double, lon: Double, unit: String) =
        getResult { apiService.getFiveDayForecast(lat, lon, unit) }
